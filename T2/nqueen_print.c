...
int main(int argc, char **argv)
{
    ...
    if ( my_rank == 0 )
    {
        ...
        printf("[Coordenador] Iniciando calculo para tabuleiro %dx%d com %d processos.\n", tamanho_tabuleiro, tamanho_tabuleiro, proc_n);
        printf("[Coordenador] Total de tarefas geradas no saco de trabalho = %d\n", total_tarefas);
        ...
        printf("[Coordenador] Enviando rajada inicial de trabalho para os %d trabalhadores.\n", proc_n - 1);
        ...
        while (temTrabalho()) {
            ...
            printf("[Coordenador] Recebi %d solucoes do trabalhador %d (total acumulado = %d).\n", solucoes_possiveis_local, status.MPI_SOURCE, solucoes_possiveis + solucoes_possiveis_local);
        ...
        printf("[Coordenador] Saco de tarefas vazio. Aguardando trabalhadores terminarem e finalizando-os.\n");
        ...
        printf("\n[Coordenador] Tempo de execucao: %f segundos\n", t2-t1);
        printf("[Coordenador] Total de solucoes possiveis encontradas = %d\n", solucoes_possiveis);
    }
    else
    {
        // Papel do escravo
        ...
        while(1){
            ...
            if (trabalho[0] == -1){
                printf("[Trabalhador %d] Recebi sinal de finalizacao. Encerrando.\n", my_rank);
                break;
            }

            printf("[Trabalhador %d] Estou trabalhando na tarefa (colunas %d, %d, %d).\n", my_rank, trabalho[0], trabalho[1], trabalho[2]);
            int resultado = trabalhar(trabalho);
            printf("[Trabalhador %d] Terminei a tarefa (colunas %d, %d, %d) e respondi %d solucoes.\n", my_rank, trabalho[0], trabalho[1], trabalho[2], resultado);
...
// === FUNÇÕES COORDENADOR ===
void mandarTrabalhoParaEscravo(int processoEscravo) {
    printf("[Coordenador] Enviei a tarefa %d (colunas %d, %d, %d) para o trabalhador %d.\n", tarefa_atual, saco_de_tarefas[tarefa_atual][0], saco_de_tarefas[tarefa_atual][1], saco_de_tarefas[tarefa_atual][2], processoEscravo);
...
void matarEscravo(int processoEscravo) {
    ...
    printf("[Coordenador] Enviei sinal de finalizacao para o trabalhador %d.\n", processoEscravo);
